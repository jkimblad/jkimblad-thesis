...
Embedded systems is everywhere around us, even though they might not be as obvious to us at first,
but they exist in things such as our cellphones, cars, washing machines, airplanes, TV's and with
the rise of the internet of things even our fridge and coffee maker. These systems can range in
complexity and they often interact in different ways with the surrounding world. While a washing
...
lines of code that monitor different systems and makes sure that the car operates as desired. One
could now imagine that a car requires a much higher degree of safety than a coffee maker in the
sense that the computers in a car cannot crash randomly or exhibit undesirable behaviour.
...
\chapter{<Result> Use a self-explaining title} \label{ch:result}
...
the memory. In all of the experiments presented in section \ref{ch:result} a \acrshort{a}-ratio and
...
however, requires a special type of precedence. The \acrshort{e}-phases are not explicitly
scheduled by the scheduler as they have nothing to do with the memory, but they are still required
to finish execution before the \acrshort{r}-phase is allowed to begin. An idea then could be to pose
a special type of precedence constraint where job 2 must not only wait for job 1 to finish, but must
also wait x amount of time after job 1 has finished before job 2 is allowed to start execution. Time
x would then be represented by the \acrshort{wcet} of the \acrshort{e}-phase similarly to how the
task model produced in the thesis works. 
