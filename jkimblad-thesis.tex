...
to have finished. This also means that the task have been assigned a processing core already. In
...
Thus all of these properties of the jobs must be properly generated in order to perform benchmarks.
...
Since the major applications of the \acrshort{aer} model lies within the automotive and avionics
...
\acrshort{r}-ratio where each value represents the ratio the respective phase to the total
...
the same as the best-case release time for all jobs. The priorities are of the jobs are then
assigned by a scheduling algorithm discussed in \ref{sec:scheduling_algorithms}. Even though
...
reached at which no more jobs is created for that task. Since all the jobs are created in pairs of
...
worst-time execution time is then assigned according to the execution time of the \acrshort{a}-phase
...
time. When all the jobs for every task has been created they are all assigned priorities according
...
effect on how many tasks needs to be scheduled when the other variables are constant. Smaller amount
...
variables and also for different ways of generating tasks, but the general affect of changing each
