...
life-saving choises. Due to this research has been focusing on developing safer models that are
easier to test and are more deterministic whithin different areas of the vehicle. One such area is
the access to shared memory by different computers on the car. As modern cars can contain over 100
different procerssors, known as \acrshort{ecu}'s, internal communication between them is a
...
(core) making it multi-core. Communication is not only necessary between \acrshort{ecu}'s but also
internally between cores on a single \acrshort{ecu}. This is done through shared memory in a
congestive way decreasing how deterministic the system is.

    %provide a review of the literature related to the topic

As an attempt to battle the problem of undeterministic memory-access the \acrshort{aer}-model has
been proposed by \textcite{durrieu_predictable_2014} for use in the avionics industry. This model
proposes a new way of scheduling programs on processesing cores to completely avoid memory
...
The \achrshort{aer}-model divides each executed program up into three distinct parts, the
\acrfull{a} phase, \acrfull{e} phase and \acrfull{r} phase. During the \acrshort{a} and \acrshort{r}
phases the program only accesses the memory. Regular programs are instead allowed to to this
whenever they like, making it more undeterministic. However, knowing that the model makes programs
more deterministic is not sufficient to increase the safety. The reason that deterministic programs
is good is because they become much easier to analyse. However changing how programs execute under
new models often require different methods of analysis.
...
\begin{itemize}
    \item Perform a state-of-the-art litterature study within the research area.
    \item Generate task-sets in a fair way.
    \item Transform the tasks into apropriate input to the analyis tool.
...
presented in \textcite{hakansson_portal_2013}. The thesis will take a quantative research approach
to try and refute or verify the presented theories. Experimental research methods will be applied
under the fundamental philosocphical assumption of positvism where facts and truths about the world
...
experimental research strategy will be used where all factors and variabels that can affect the
...
Chapter \label{ch:theoretic_background} gives a theoretic background to the problem area
such that the reader becomes equipped to understand the topic at hand in-depth.
...
%TODO: Explain why the input is chosen the way it will be, 10% a and e cycles, the task amount, the
%core amount etc.
