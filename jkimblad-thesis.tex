...
% TODO: maybe talk about aperiodic and sporadic tasks as well? 
...
%TODO: Mention that the critical instance (when all tasks are released at the same time) is not
%enough to evaluate if the task set is schedulable when considering a multi-core system.
...
% TODO Reference original image in caption
\begin{figure}
...
% TODO: Add that some task sets (with tasks that have a phase) does not have a critical instant for
% all tasks. 
...
% TODO add graph showing necessary and sufficient conditions coverage of schedulable task sets (see
% zim), then explain research is about shrinking window between them while also keeping a low O(n).

\section{The AER Execution Model}

% TODO: mention that a pre-requisite of AER is separate memory banks for each core
...
is done by dividing up each task into three distinct parts. The \textbf{acquisition} part reads all
the data necessary for execution of the task such as code and shared variables into the private
memory of the core dedicated for execution. The \textbf{execution} phase then executes the code,
without making any further read or write instructions to the shared memory, operating on local
copies of the shared variables. The \textbf{restitution} phase then writes back all of the acquired
results of the task to the shared memory.

%Mention that a requirement of the AER model is that the code and variables of each task all fit
%within a private memory bank. I.e, tasks can not be to big!
...
state-of-the-art schedulability analysis method for non-preemptive static tasks sets under global
...
\acrshort{prem} such as \ref{alhammad_schedulability_2014} which borrows the notion of a problem
window from \ref{baker_multiprocessor_2003} which presents an analysis method for \acrshort{edf} and
\acrshort{dms} on multi-core processors. 
 
\begin{itemize}
    \item Schedulability analysis paper, this is the most important
        \item The 2 schedulability analysis papers that this one compares it with
    \item The schedulability analysis paper for single core?
    \item Matthias paper
\end{itemize}
