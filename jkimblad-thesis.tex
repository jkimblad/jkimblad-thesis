...
\begin{abstract} 
    ...
    \acrshort{aer}-model and the developed \acrshort{iip} are then benchmarked with task sets coherent to the
...
to the rise of the internet of things also in our fridges and coffee makers. These systems often
...
car crash randomly or exhibit undesirable behavior they may lead to much more severe outcomes.
...
skepticism of their abilities to make life-saving choices. This of course is one of the reasons
to the increasing amounts of lines of code as well as the increasing amount of computers in cars.
Due to increased complexity in these systems research has been focusing on developing safer models
that are easier to test and that are more deterministic. One research area with interesting results 
is to examine the access of shared memory by different computers in the car. As modern cars can 
contain over 100 different processors, known as \acrfull{ecu}'s, internal communication between them 
is a necessity and often use some type of databus technology such as a \acrfull{can}-bus.
Communication is then not only necessary between \acrshort{ecu}'s but also internally
between cores on a single \acrshort{ecu}. This is done non-deterministically through shared memory
...
The existing problem is that timing-analysis is hard and needs to be customized for
different ways of scheduling tasks. Analyzing embedded systems is hard but very necessary,
...
like the \acrshort{aer}-model.  Although some techniques do exist to analyze scheduling under the
...
The theoretical research methods and methodologies have been chosen to follow the guidelines
presented by \textcite{hakansson_portal_2013}. The thesis will take a quantitative research approach
to try to refute or verify the presented theories. Experimental research methods will be applied
...
predetermined windows as parts of the tasks whole scheduling window within which they are allowed
...
will either perform the same (being also exact) or simply more pessimistic while analyzing the same
...
for faulty behavior which leads to fewer system failures that may cause harm to humans. As
...
software disconnected and independent of any parts causing emissions by burning fossil fuels. The
...
all the new concepts and work that has been developed in this thesis and the methods from which
...
When used in a real product it is important that a scheduling algorithm is able to schedule a task
set without any missed deadlines, section \ref{sec:schedulability_analysis} covers how schedulers
can be analyzed to make sure this requirement is fulfilled. Section \ref{sec:np-test} presents the
most important paper which this thesis build onto, which is a schedulability analysis tool. Section
\ref{sec:aer_execution_model} presents an execution model used to combat nondeterministic behavior
using scheduling in embedded systems caused by shared memory and section \ref{sec:related_work}
...
again, this is known as the tasks \textbf{period}, denoted by $T_i$. The earliest time when a task
is ready to start execution within a new period is called the \textbf{arrival time} of the task and
is denoted $ a_i $.  For all purposes in this thesis the task arrival times can be assumed to be the
...
concepts are introduced. Computation time of a task varies because of unpredictable behavior in the
...
of the earliest arrival time, denoted $ a_i^{min} $, is the  absolute earliest time at which a task may
...
ongoing tasks in and out of the core during run time. For the \acrshort{os} to switch tasks quickly
...
to resume execution of $T_3$ at time 5. This is what is meant by a preemptive scheduling algorithm.
...
scheduling algorithm can be used individually on each core during run time. It is then possible to
...
find close to optimal solutions than if it had to be done during run time.
...
\acrshort{edf}, can be applied to global scheduling, but they will have different behavior. Global
...
All the algorithms explored this far have taken a \textbf{core-centric} approach to scheduling tasks,
...
missed deadlines could mean fatal outcome. For example, \acrshort{rms} can be analyzed
...
are of $ O(n^3) $ complexity which can be alright when analyzing small task sets, but is too complex
as the task sets grow or if it has to be calculated during run time with limited computation power.
...
only a small part of the schedule needs to be analyzed.
...
It is intuitive to think that it is sufficient to analyze the worst-case times of all tasks to find
...
this phenomenon can be seen in figures \ref{fig:np-test_schedule} and \ref{fig:np-test_schedule_fail} which is also further explored in the next
section.
...
example perfectly shows the problem with analyzing non-preemptive scheduling as it is not sufficient
...
access phase and an execution phase. Since the bottleneck for utilization in multi-core systems can
...
instructions to the shared memory. The \textbf{restitution} \acrshort{r}-phase then writes back all the
acquired results that need to be shared to the shared memory. Since all the instructions are read
during the \acrshort{a}-phase before the \acrshort{e}-phase starts its execution a prerequisite of
the model
is that each core has a separate private memory, such as a cache or a scratchpad. Since these types
of memories are often constrained in size it puts a limit to how many instructions a task can
...
analyze. Timing that was calculated offline, for example the \acrshort{wcet}, may not match reality
...
Some work on analyzing the schedulability of algorithms developed around
...
executing \acrshort{e}-phases}" \parencite{maia_schedulability_2017}. The amount of time within all the bus
holes inside the problem window must be enough for the task being examined to schedule its \acrshort{a} and \acrshort{r}
...
each compute cluster one memory bank is reserved as the shared memory which stores all the
...
whether the input is schedulable. The basic idea of the analysis is to enumerate all of
...
begins right after the first and is the size of the \acrshort{wcet} taken from the \acrshort{e}-phase as it
...
the \acrshort{r}-phase. Each of the job's arrival time and deadline are then set to correspond to its
...
\acrshort{iip} makes a decision about whether the highest priority job is to be scheduled
when the core becomes free or if the core should be idle until another higher priority job is
released.

For a basic \acrshort{iip} to make a decision about whether to schedule a job it is provided
...
whether the memory is free. The \acrshort{np}-test can however be used in conjunction with
...
\acrshort{a}-job. And secondly if it is an \acrshort{a}-job there has to be a check if there is a
...
\acrshort{aer} \acrshort{iip}, which is the total amount of cores in the system (whether they are free). 
To find out the amount of free cores in the system an additional algorithm is
...
argues that prioritizing \acrshort{r}-jobs will free up cores and hinder blockage in the system and
also mentions that the \acrshort{r}-job often will be closer to the task's deadline making it more
...
to be used is guaranteed to be fulfilled, as it is a criterion for the scheduler to be invoked. There
...
\acrshort{iip} is called with an \acrshort{r}-job it is guaranteed that the job
...
Testing of software is always important to make sure it exhibits the desired behavior. As the
...
c++ testing framework known as doctest by the authors. However, to test the developed software a
...
represent both the average-case and edge-cases to increase coverage and the possibility of catching
undesired behavior and bugs in the code.
...
There are several interesting ways to prioritize the jobs. Existing scheduling algorithms
...
period, thus their jobs will have the same priority, so it is of interest to further prioritize the
jobs which share the same period according to some tiebreaker. The implementation of
\acrshort{np}-test tie-brakes priorities based on task-id and job-id where a lower id is given
a higher priority. A more interesting idea is to prioritize jobs with the same period according
...
a lot of jobs may end up with the same priority, the jobs of the same period are also prioritized using
the jobs' utilizations as a tiebreaker. They can in a way be thought of as special versions of
...
\begin{itemize}
    ...
    \item \textbf{\acrfull{lulp}} - Initially priorities are given according to \acrshort{rms}
        ...
        regular \acrshort{rms} jobs with the same period are further prioritized according to their
    ...
    \item \textbf{\acrfull{hulp}} - Works just like \acrshort{lulp} but jobs with the same period
        are instead prioritized in a way where the jobs with the highest utilizations always are
    ...
    \item \textbf{\acrfull{huhp}} - Work just like \acrshort{luhp} but jobs with the same period are
        instead prioritized by giving the jobs with the highest utilizations the highest priorities
        of all jobs with the same period.
...
and a job is depicted by a box. It is important to remember that all the \acrshort{r}-jobs must
have a higher priority than all the \acrshort{a}-jobs as described in section \ref{sec:iip}. As
...
To benchmark the extended \acrshort{aer}-model and the \acrshort{aer} \acrshort{iip} job sets
have to be generated to adhere to the input restrictions of the \acrshort{np}-test. The input to the
...
Thus, all the properties of each job must be properly generated in order to perform benchmarks.
...
phases are depicted in figure \ref{fig:benchmark_toolchain}. Decoupling is done such that each of the three
phases can be switched out to a counterpart without having to make any changes to
...
\begin{figure}
    ...
    \caption{The toolchain developed to generate input to the \acrshort{np}-test.}
...
The paper by \textcite{bini_measuring_2005} goes into great detail explaining some common pitfalls
...
from randomizing a period and pairing it with a random utilization from the set created by the
...
After pairing a utilization with a period to create the task's execution time the question remains
...
used when creating the job set and setting it to something different from what was generated by the
...
for the phases and it is not trivial. Information about the execution times of the acquisition and restitution phases of 18
different tasks in an engine management system in a car can be found in the paper by \textcite{becker_scheduling_2018}.
These could be used to make approximations
...
an \acrshort{a}-job and an \acrshort{r}-job where the \acrshort{a}-job is created first and given an odd
...
created similarly, but given an even job-id. Its release time is calculated as the
...
task. The deadline is then assigned according to the task's deadline, but transformed into absolute
...
The experiments will be running on an "HP Z210 Workstation" with a quad-core Intel Core i7-2600 CPU
\@3.40GHz and 16 GB of memory.
...
ratio between schedulable task sets and the total amount of task sets tested. All the independent
...
for both of the experiments to create the schedulability ratio. Since the run time rises rapidly
with the size of the task set the iterations are set to a lower amount than for some other experiments.
...
also be tested what the effects are when the amount of cores is a possible bottleneck. This will be
...
total size allocated to the \acrshort{a}-window, where all the remaining percentages are
...
All the window ratios have a quick initial fall of in schedulability before it planes out to a
...
better than all others, but during lower utilizations although it being among the top ones it is
...
\begin{figure}
    ...
    \caption{The schedulability ratios of each tested window ratio averaged out over the amount of different utilizations.}
...
\acrshort{r}-phase (a high window ratio) seems to favor the average schedulability ratio compared to
giving the same window size to the \acrshort{a}-phase.
...
\begin{figure}[H]
    ...
    \caption{The schedulability ratio as an effect of increasing the amount of tasks while utilization is constant between 0 and 1000 tasks.}
...
\begin{figure}
    ...
    \caption{The schedulability ratio as an effect of increasing the amount of tasks while utilization is constant between 0 and 250 tasks.}
...
\begin{figure}
    ...
    \caption{The schedulability ratio of different core amounts as an effect of the total
    	utilization of the task set.}
...
the steady decline in schedulability ratio as the amount of cores is decreased.
...
\begin{figure}
    ...
    \caption{The schedulability ratios of each tested core amount averaged out over the amount of different utilizations.}
...
\ref{fig:sched_algos}. All the algorithms had a quick initial drop off in schedulability ratio
...
\begin{figure}[H]
    ...
    \caption{The schedulability ratio of different scheduling algorithms as an effect of the total
    	utilization of the task set.}
...
\begin{figure}[H]
    ...
    \caption{The schedulability ratios of each tested scheduling algorithm averaged out over the amount of different utilizations.}
...
This chapter concludes the thesis by summarizing its findings from benchmarking and the results in
...
scheduling algorithms are designed to always give all the \acrshort{r}-jobs a higher
...
\acrshort{r}-jobs on average will have a larger slack time than the \acrshort{a}-jobs, some
...
deadline. This problem was hypothesized and explained in section \ref{subsec:task_generation} which
...
highest average schedulability ratios after \acrshort{edf} makes it evident that prioritizing jobs with a
...
The results also show that prioritizing jobs within their periods according to their utilization
...
(10ms or 20ms with a distribution totaling 29\% of tasks, see table \ref{tab:period_distribution}) is on
...
cores is equal to the amount of tasks, which means that each task has a dedicated core to execute
...
matter when the amount of tasks is the same or smaller than the amount of cores, but should be
expected to become a larger problem when the task set grows or the amount of cores is lowered
...
the memory. In all the experiments presented in chapter \ref{ch:results} an \acrshort{a}-ratio and
...
be 20\% of this equaling a utilization of 0.2.

As discussed in section \ref{subsec:running_benchmarks}, there are a lot of different independent
variables that all affects the schedulability ratio. Even though a lot of different values are tried
for the different variables to show their general effect on the schedulability ratio it is a very
...
when the amount of cores exceeds the amount of tasks, which is not realistic in all cases, but the
performance is quickly decreased when the amount of cores is decreased below the amount of tasks.
...
type of precedence constraint where an \acrshort{r}-job must not only wait for its \acrshort{a}-job
...
more tasks with the same period as the amount of cores to be schedulable.
...
upon by modeling the \acrshort{wcet} and \acrshort{bcet} of the \acrshort{e}-job as release jitter
...
\acrshort{a}-jobs and \acrshort{r}-jobs are always constant in time is possible by modeling them
