...
The \acrshort{aer}-model divides each executed program up into three distinct parts, the \acrfull{a}
phase, \acrfull{e} phase and \acrfull{r} phase. During the \acrshort{a} and \acrshort{r} phases the
program only accesses the memory. Regular programs are instead allowed to to this whenever they
like, making it more undeterministic. However, knowing that the model makes programs more
deterministic is not sufficient to increase the safety. The reason that deterministic programs is
good is because they become much easier to analyse. However changing how programs execute under new
models often require different methods of analysis.
