...
This is only a sufficient schedulability test, meaning that \acrshort{rms} might be able to schedule
task sets that do not fulfill the constraint, but it is not guaranteed to. \acrshort{rms} does not
guarantee finding a feasible schedule if such a schedule exists, making it a \textbf{heuristic}
scheduling algorithm. In contrast \acrshort{edf} has been proven to always find a feasible schedule
for preemptive single-core systems if one exists, making it \textbf{optimal} in those circumstances.
Under the assumptions of a preemptive single-core system \acrshort{edf} is able to schedule task
sets with a processor utilization factor of one. 

Another way of analysing a schedule is simply to simulate it and look for missed deadlines. 

% TODO think about moving this paragraph to the task model instead.
Since the task model presented in \ref{sec:the_task_model} is very simplistic some additional
concepts are introduced in order for it to better represent reality to increase the validity of
schedulability analysis methods. The computation time of a task varies because of unpredictable
behaviour in the processor such as caches. Therefor the notions of \acrfull{bcet} and \acrfull{wcet}
is introduced to represent the interval between the fastest respective slowest computation time of a
specific task. To denote this $ C_i^{min} $ and $ C_i^{max} $ is used. In addition to uncertainties
in the computation time there are also uncertainties in the arrival time of tasks. The notion of
earliest arrival time, denoted $ a_i^{min} $, is the  absolute earliest time at which a task may be
able to start executing within the period and likewise the latest arrival time, denoted $ a_i~{max}
$ is the time when the task is guaranteed to be able to start execution.
...
deadline later in the schedule.
